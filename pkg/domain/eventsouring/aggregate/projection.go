...
type Update func(context.Context, []*vo.Statement, Reduce) (unexecutedStmts []*vo.Statement, err error)
...
type SearchQuery func(ctx context.Context) (query squirrel.SelectBuilder, queryLimit uint64, err error)
...
func NewProjectionHandler(config ProjectionOptions) *ProjectionHandler {
	h := &ProjectionHandler{
		Handler:        NewHandler(config.HandlerOptions),
...
func (h *ProjectionHandler) Process(
	...
	for {
		...
		case <-ctx.Done():
			...
			h.shutdown()
			return
		case e := <-h.Handler.EventQueue:
			if err := h.processEvent(ctx, e, reduce); err != nil {
...
func (h *ProjectionHandler) bulk(
	...
	errs := lock(ctx, h.requeueAfter, "system")
